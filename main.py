...
def search_person(age: Optional[int] = Query(None, title="Age"), 
                  name: Optional[str] = Query(None, title="Name")):
    consulted_person = [p for p in people if p["age"] == age]

    if name is None:
        if age is None:
            return people
        else:
            return consulted_person
    else:
        consulted_person_alt = [p for p in people if name.lower() in p["name"].lower()]
        if age is None:
            return consulted_person_alt
        else:
            combined = [p for p in consulted_person if p in consulted_person_alt]
            return combined
...
def change_person(person: Person):
    ...
    if len(person_list) > 0:
        ...
        return HTTPException(status_code=404, detail=f"Person with id {person.id} doesnt exists")
